...
int main(void)
{
  ...
  uint8_t dataToSend[8] = {0x56, 0x26, 0x88, 0x12, 0x67, 0x99, 0xAA, 0x88};
  uint16_t msgId_1 = 0x200;

  uint8_t msgId[4];
  msgId[0] = (msgId_1 >> 3) & 0xFF;  // Estrai il byte alto (MSB)
  msgId[1] = (msgId_1 << 5) & 0xE0;         // Estrai il byte basso (LSB)
  ...
  while (1)
  {
    ...
    if (result_mcp2515Init == MCP2515_OK){
      MCP2515_InterruptHandler(&mcp2515_1, &mcp2515Int, &canMsg_buffer);
      MCP2515_SendMessage(&mcp2515_1, &canMsg_buffer, dataToSend, msgId);
    }
    /* USER CODE END WHILE */
